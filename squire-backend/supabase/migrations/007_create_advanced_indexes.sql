...
-- Partial indexes for non-expired suggestions (removed NOW() for immutability)
...
-- Expression indexes for JSONB queries (using immutable operators)
CREATE INDEX idx_user_profiles_preferences_notifications ON user_profiles((preferences->>'notifications'))
    WHERE preferences ? 'notifications';

CREATE INDEX idx_session_data_app_name ON user_sessions((session_data->>'app_name'))
    WHERE session_data ? 'app_name';

CREATE INDEX idx_suggestions_suggestion_title ON ai_suggestions((suggestion_content->>'title'))
    WHERE suggestion_content ? 'title';
